...
<script>
	$(document).ready(function() {
		//history.replaceState({}, null, "http://localhost:9090/tumblbugs/" + '${vo.pj_addr}');
	...
	<div id="project_content">
					...
					<div>
						<c:choose>
							<c:when test="${vo.remaining_days >= 0 }">
								<a href="http://localhost:9090/tumblbugs/funding/step1?pj_id=${vo.pj_id }">
		...
		<section id="menu_tab">
			<nav>
				...
				<a id="community" href="http://localhost:9090/tumblbugs/project_content?tab=community&pj_id=${vo.pj_id }">커뮤니티
					<c:if test="${vo.community_count ne '0'}">
						<span id="community_count">${vo.community_count }</span>
					</c:if>
				</a>
				<a id="fundingInfo" href="http://localhost:9090/tumblbugs/project_content?tab=policy&pj_id=${vo.pj_id }">펀딩 안내</a>
			</nav>
		</section>
				...
				<aside>
					...
					<c:forEach items="${giftList }" var="gift">
						<div class="white_box" id="present_option">
							...
							<c:choose>
								<c:when test="${vo.remaining_days >= 0}">
									<a href="http://localhost:9090/tumblbugs/funding/step1?pj_id=${vo.pj_id }&gift_id=${gift.gift_id}">
