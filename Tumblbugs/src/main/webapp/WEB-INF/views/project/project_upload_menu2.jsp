...
<div class="content_menu_title">펀딩 목표 설정</div>
				...
				<label>프로젝트 시작일</label>
					<div id="start_info" class="sub_info">
						<c:if test="${vo.pj_start_date != null}">
							<span style="color:black; font-size:11pt; font-weight:550;">${vo.pj_start_date}</span>
						</c:if>
						<c:if test="${vo.pj_start_date == null}">
	...
	<div id="pro_settle_date">
		<div id="content_m2_c3_d" class="pay_date">	
			<c:if test="${vo.pj_pay_end_date == null && vo.pj_bill_date == null}"><div id="null_pay_date"><div class="p_title">예상 정산일</div> 마감일을 입력하시면 예상 정산일을 안내드립니다. </div></c:if>
			<div id="pay_end_date"><c:if test="${vo.pj_pay_end_date != null}"><span class="bold_txt">결제 종료일 :</span>  펀딩에 성공할 경우 <label class="bold_text">마감일 다음날부터 7일간 결제가 진행</label>되어,  
															<label class="bold_text">${vo.pj_pay_end_date }</label> 모든 후원자의 결제가 종료됩니다.</c:if>
			</div>
			<div id="bill_date"><c:if test="${vo.pj_bill_date != null}"><span class="bold_txt">예상 정산일 :</span> 
				 결제 종료일로부터(공휴일 및 주말 제외한 영업일 기준) 7일 이후인, <label class="bold_text">${vo.pj_bill_date }</label>에 창작자님의 계좌로 입금됩니다.</c:if>
			</div>
		</div>	
	</div>
...
<input type="hidden" value="1000" id="present_number">
	...
	<div id="pro_present1" class="pro_present">
		...
		<div id="content_m2_c4_d" class="upload_detail">
	...
	<div id="pro_present1" class="pro_present">
			...
			<div id="item_option_div">
				<c:forEach var="list" items="${glist}">
					<c:forEach var="ivo" items="${list.itemList}">
						<c:if test="${ivo.gift_id == gvo.gift_id && ivo.item_check == 'y'}" >
							<div>- ${ivo.item_title}(x ${ivo.item_count })</div>
						</c:if>
					</c:forEach>
				</c:forEach>
			</div>
		...
		<div id="content_m2_c4_d${gvo.rno}" class="upload_detail">
			...
			<div>
				...
				<div id="m2_c4_add_items">
					...
					<table class="add_items_table">
						...
						<c:forEach var="list" items="${glist}">
							<c:forEach var="ivo" items="${list.itemList}">
								<c:if test="${ivo.gift_id == gvo.gift_id}" >
									<tr id='item_number${list.rno }${ivo.rno}' value="${ivo.item_id }"
										...
										<td id='item_option'>${ivo.item_option }</td>
										<td id='item_count_td${ivo.rno}'>
											<i class='fas fa-minus-circle' id='itmes_count_minus'></i>
											<span><input type='text' id='pro_items_count"+item_value+"' value='${ivo.item_count }' class='pro_items_count' readonly></span>
											<i class='fas fa-plus-circle' id='itmes_count_plus'></i>
											<input type='hidden' id='item_option_type' value='${ivo.item_option_type}'>
											<input type='hidden' id='item_id' value='${ivo.item_id }'>
										</td>
									</tr>
			...
			<div>
				...
				<div class="content_btn">
					<input type="hidden" class="gift_id" value="${gvo.gift_id}">
					<button type="button" class="gift_reset"><i class="fas fa-times"></i> 취소하기</button>
					<button type="button" class="update_present_btn" id="update_present_btn"><i class="fas fa-check"></i> 수정하기</button>
				</div>		
			</div>
		</div>
	</c:forEach>
<!-- ------------------- -->	
					...
					<div id="pro_funding_info">
						...
						<div id="content_m2_c5_d" class="upload_detail">
							...
							<div><textarea class="m2_check" id="m2_c5_i1"><c:if test="${vo.pj_refund == null}">[ - 모든 프로젝트 공통 ]
				...
				<div id="item_list">
					...
					<div class="item_table_class" id="item_table_div">
						...
						<table id="item_table">
							...
							<c:forEach var="list" items="${glist}">
								<c:forEach var="ivo" items="${list.itemList}" >
									<tr id='list_item_number${list.rno}${ivo.rno}' >
										<td>${ivo.item_title }</td>
										<td>${ivo.item_option }</td>
										<td><button type='button' class='list_update'>수정하기</button>
											 <button type='button' class='list_remove'>삭제하기</button>
											 <input type='hidden' class="option_type" id='item_option${ivo.item_option_type}' value='item_number${list.rno}${ivo.rno}'>
											 <input type='hidden' id='product_number' value='#content_m2_c4_d${list.rno}'>
											 <input type='hidden' class='modal_item_id' id='item_number${ivo.rno}' value='${ivo.item_id}'>
										</td>
									</tr>
								</c:forEach>
							</c:forEach>
